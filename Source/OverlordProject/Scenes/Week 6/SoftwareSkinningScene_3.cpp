...
void SoftwareSkinningScene_3::Update()
{
	...
	for (int i = 0; i < m_SkinnedVertices.size(); i++)
	{
		...
		if (m_SkinnedVertices[i].blendWeight0 == 0.f)
		{
			XMStoreFloat3(&finalPos, XMVector3TransformCoord(originalPosVec, bone1Transform));
		}
		else if (m_SkinnedVertices[i].blendWeight1 == 0.f)
		{
			XMStoreFloat3(&finalPos, XMVector3TransformCoord(originalPosVec, bone0Transform));
		}
		else
		{
			XMVECTOR bone0FinalPosVec = XMVector3TransformCoord(originalPosVec, bone0Transform);
			XMVECTOR bone1FinalPosVec = XMVector3TransformCoord(originalPosVec, bone1Transform);
			auto finalPosVec = bone0FinalPosVec * m_SkinnedVertices[i].blendWeight0 + bone1FinalPosVec * m_SkinnedVertices[i].blendWeight1;
