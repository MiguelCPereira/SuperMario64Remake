...
void Star::TogglePause(bool paused)
{
	if (paused)
	{
		...
		if (m_Paused == false)
		{
			m_pPickedUpChannel->getPaused(&m_PickedUpPlaying);
			m_pIdleChannel->setPaused(true);
			m_pPickedUpChannel->setPaused(true);
			m_Paused = true;
		}
	...
	{
		m_pIdleChannel->setPaused(m_IdlePlaying);
		m_pPickedUpChannel->setPaused(m_PickedUpPlaying);
		m_Paused = false;
	}
}
